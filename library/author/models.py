...
class Author(models.Model):
    ...
    def get_by_id(author_id):
        ...
        """
        try:
            result = Author.objects.get(id=author_id)
            return result
        except:
            return None
    ...
    def create(name, surname, patronymic):
        ...
        """
        if len(name) <= 20 and len(surname) <= 20 and len(patronymic) <= 20:
            new_author = Author.objects.create(name=name, surname=surname, patronymic=patronymic)
            return new_author
        else:
            return None
